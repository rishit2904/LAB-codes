...
__global__ void matrixMultiplyRow(int *A, int *B, int *C, int n) {
...
__global__ void matrixMultiplyCol(int *A, int *B, int *C, int n) {
...
__global__ void matrixMultiplyElement(int *A, int *B, int *C, int n) {
...
int main() {
    ...
    matrixMultiplyRow<<<N, 1>>>(d_A, d_B, d_C_row, N);
    cudaMemcpy(h_C, d_C_row, size, cudaMemcpyDeviceToHost);
    printMatrix("Matrix Multiplication (Row-wise Computation):", h_C);

    // 2. Column-wise computation
    matrixMultiplyCol<<<N, 1>>>(d_A, d_B, d_C_col, N);
    cudaMemcpy(h_C, d_C_col, size, cudaMemcpyDeviceToHost);
    printMatrix("Matrix Multiplication (Column-wise Computation):", h_C);

    // 3. Element-wise computation
    dim3 threadsPerBlock(16, 16);
    dim3 blocksPerGrid((N + 15) / 16, (N + 15) / 16);
    matrixMultiplyElement<<<blocksPerGrid, threadsPerBlock>>>(d_A, d_B, d_C_element, N);
    cudaMemcpy(h_C, d_C_element, size, cudaMemcpyDeviceToHost);
    printMatrix("Matrix Multiplication (Element-wise Computation):", h_C);
